 ...
 *
 *         The hook should be owned by the governance that owns the PAU, and its pools are
 *         permissionless, so pool lifecycle and configuration are operated directly by the owner
 *         rather than through this facet, and the ALMProxy enters and exits as a regular LP. Swaps
 *         through DualPool pools go through the UniswapV4 facet's swap, which takes the PoolKey
 *         from calldata.
 *
