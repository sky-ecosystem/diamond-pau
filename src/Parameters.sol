...
import { IParameterErrors } from "./interfaces/IParameterErrors.sol";
...
library Parameters {
    ...
    function toBool(bytes32 parameter) internal pure returns (bool value) {
        if (uint256(parameter) > uint256(1)) revert IParameterErrors.ParameterOutOfTypeBounds();
    ...
    function toAddress(bytes32 parameter) internal pure returns (address value) {
        if (uint256(parameter) > type(uint160).max) {
            revert IParameterErrors.ParameterOutOfTypeBounds();
        }
    ...
    function _checkBytes(bytes32 parameter, uint256 max) private pure {
        if (uint256(parameter) > max) revert IParameterErrors.ParameterOutOfTypeBounds();
    }

    function _checkInt(bytes32 parameter, int256 min, int256 max) private pure {
        if (int256(uint256(parameter)) > max) revert IParameterErrors.ParameterOutOfTypeBounds();

        if (int256(uint256(parameter)) < min) revert IParameterErrors.ParameterOutOfTypeBounds();
    }
