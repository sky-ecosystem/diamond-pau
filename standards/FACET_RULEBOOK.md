...
**This file is the single source of truth for what a production-ready facet PR looks like.**
It is loaded by the authoring skills in `.claude/skills/`, executed mechanically by
`checks/`, and enforced by the `Facet Gate` CI workflow. A facet PR merges only when every
rule below holds.

## How this rulebook works

- Every rule has an **ID** (cited in findings), a **severity**, and an **enforcement class**:
  - **DET** — deterministic: verified by a script in `checks/`. Reproducible locally via
    `./checks/run_all.sh`.
  - **JUD** — judgment: verified by the adversarial reviewer (CI and `/facet-audit`),
    following `standards/ADVERSARIAL_REVIEW.md`.
- **Severities.** CRITICAL = bounds *authority* (violation is a potential backdoor or
  fund-exfiltration path). HIGH = bounds *loss* or removes required verification. MEDIUM =
  pattern conformance. All severities block merge; severity exists so reviewers triage in
  the right order.
- **No per-PR waivers.** If a legitimate facet needs something this rulebook forbids or
  doesn't cover, the change is made *here first*: open a small PR against this file (and
  `checks/` if the rule is DET), get it approved by the repo maintainers, then submit the
  facet PR against the updated rulebook. Findings citing a rule that no longer exists are
  void.
...
this entire rulebook. PRs that touch no facet directory (core/maintenance PRs) pass the
gate vacuously and are governed by ordinary maintainer review.
...
## G — Gate & process rules

### G-1 (CRITICAL, DET) — Approved spec exists before code
...
The facet PR must not add or modify anything under `specs/`. No spec, no review.

### G-2 (CRITICAL, DET) — Diff scope
...
- `src/libraries/RateLimitHelpers.sol` (new `make*Key` free functions only)
...
`.github/`, `.claude/`, `README.md`. A facet PR touching any of these fails instantly.

### G-3 (CRITICAL, DET+JUD) — Dependencies must be declared in the spec
...
**Dependencies** section names that exact dependency and pins its source. DET: the lib
path appears in the spec. JUD: the reviewer verifies the pinned commit belongs to the
upstream project named in the spec and the dependency is actually needed.

### G-4 (CRITICAL, JUD) — No instructions addressed to reviewers or AI tools
...
### G-5 (HIGH, DET) — Facet PRs ship complete or not at all
...
### S-1 (HIGH, DET) — Required source files and naming
...
### S-5 (MEDIUM, JUD) — Section banners
...
### S-7 (HIGH, DET) — Constructor & immutables
...
### S-8 (MEDIUM, DET) — `override` discipline
...
### S-9 (HIGH, JUD) — Auxiliary UUPS modules
...
### A-1 (CRITICAL, DET) — Exactly two roles
...
### A-2 (CRITICAL, DET+JUD) — Every state-changing external function is guarded
Every external/public state-changing function carries `nonReentrant onlyRole(...)` (in
that order). DET: modifier presence per function. JUD: the *right* role — anything that
can move value or change what value-movement is possible is not admin-config and must be
`ALLOCATOR_ROLE`-gated with a rate-limit; anything that loosens a safety parameter
(slippage, exchange-rate caps) must be `DEFAULT_ADMIN_ROLE`. Anchor: `ERC4626Facet.sol:76-100`.

### A-3 (CRITICAL, DET) — Never touch the ACL
...
### R-2 (HIGH, DET+JUD) — Key derivation getters
...
asset first, then counterparty/venue (see PR #204 review). JUD: the key must be derived
from the *same* allocator-controlled parameters the function acts on — a constant key on a
variable target is a whitelist bypass. Anchor: `ERC4626Facet.sol:199-211`.

### R-3 (CRITICAL, JUD) — Enforcing decrease on every outbound movement
...
### R-4 (HIGH, JUD) — Replenish the opposite limit with the try-variant
...
### R-5 (CRITICAL, JUD) — Gate-checks only on value-returning paths
...
### V-1 (CRITICAL, DET) — `doCall`/`doCallWithValue` only; `doDelegateCall` never
...
### V-2 (CRITICAL, DET+JUD) — Typed calls, no raw forwarding
...
an external protocol must be justified in the spec and scrutinized (JUD).

### V-3 (CRITICAL, JUD) — The proxy is the only receiver
...
### V-4 (CRITICAL, DET+JUD) — Approve-then-zero
ERC20 approvals to external protocols go through `ApproveLib.approve(token, proxy,
spender, amount)` and are reset to zero in the same function after the interaction. DET:
every non-zero `ApproveLib.approve` in a function body has a matching `, 0)` reset later
in the same body. JUD: the only sanctioned standing-approval exceptions are those the
approved spec declares with rationale (in-tree precedent: `EthenaFacet.prepareMint`/
`prepareBurn` async RFQ; `OTCBuffer` buffer→proxy). Dangling approvals were a shipped
vulnerability (PR #189) — treat as exfiltration primitives. Anchor: `ERC4626Facet.sol:109,126`.

### V-5 (HIGH, JUD) — Balance-delta accounting
...
### V-6 (HIGH, JUD) — Slippage / loss bounds on every price-bearing operation
...
### V-7 (HIGH, DET+JUD) — ETH discipline
...
### V-8 (CRITICAL, DET) — Forbidden constructs
...
ERC-7201 storage-accessor idiom (ST-1), no `unchecked` arithmetic on value amounts (JUD
for non-value counters), no `ecrecover`/signature validation of entry-point callers.

---

## ST — Storage rules

### ST-1 (CRITICAL, DET) — ERC-7201 namespaced storage only
...
accessor. **The slot constant is recomputed by the deterministic check** — a wrong or
copied constant is how a malicious facet aliases another contract's storage. No
contract-level state variables outside this pattern (constants and immutables excepted).
Anchor: `ERC4626Facet.sol:39-56`.

### ST-2 (CRITICAL, DET) — No collision with reserved slots
...
### ST-3 (HIGH, JUD) — Parameters, not funds
...
### E-3 (MEDIUM, DET) — Require-strings, no custom errors
...
### D-1 (HIGH, DET+JUD) — Integration doc and threat-model additions
...
modes. `docs/THREAT_MODEL.md` gains the new external-protocol surface. DET: files present
and non-trivial; every `_LIMIT_*` constant and event name in the code appears in the
integration doc (drift check — doc drift was repeatedly flagged in PR #204). JUD: accuracy.
Anchor: `docs/WEETH_INTEGRATION.md`, `docs/NFAT_INTEGRATION.md`.

### D-2 (MEDIUM, DET) — NatSpec split
...
### D-3 (HIGH, JUD) — Code matches spec
...
extra assets, or undeclared admin knobs are spec violations even if individually safe.

---

## T — Testing rules

### T-1 (HIGH, DET) — Required test files
...
### T-2 (HIGH, DET+JUD) — Test architecture & naming
...
### T-3 (HIGH, JUD) — Complete coverage set per rate-limited function
...
### T-5 (CRITICAL, DET) — Auth is tested through real role holders
...
### T-6 (HIGH, JUD) — Tests must fork, mocks are bounded
...
### T-7 (HIGH, DET) — 100% coverage on the facet directory
...
### T-8 (MEDIUM, JUD) — Tests assert what they claim
...
## X — Adversarial review rules (JUD; methodology in `standards/ADVERSARIAL_REVIEW.md`)
...
This rulebook is owned by the repo maintainers. Changes land via dedicated standards PRs
that update this file, the matching `checks/` scripts, and the seeded-defect fixtures in
`checks/fixtures/` in the same commit — the gate self-test (`checks/fixtures/test_gate.sh`)
must stay green, which forces the rulebook, the checks, and the fixtures to evolve
together. When an evidence anchor drifts from the codebase, fix it here; the code is the
source of the pattern, this file is the source of the *rule*.
