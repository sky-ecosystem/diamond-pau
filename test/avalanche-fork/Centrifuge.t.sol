...
abstract contract Centrifuge_TestBase is ForkTestBase {
    ...
    address internal constant CENTRIFUGE_VAULT = 0xCF4C60066aAB54b3f750F94c2a06046d5466Ccf9; // deJAAA USDC Vault

    // Requests for Centrifuge pools are non-fungible and all have ID = 0
    uint256 internal constant REQUEST_ID = 0;

    ICentrifugeV3VaultLike internal centrifugeV3Vault = ICentrifugeV3VaultLike(CENTRIFUGE_VAULT);
...
contract ForeignController_Centrifuge_RequestDepositERC7540_Tests is Centrifuge_TestBase {
    ...
    function setUp() public override {
        ...
        deal(address(usdcAvalanche), almProxy, 1_000_000e6);
    }
    ...
    function test_requestDepositERC7540() external {
        ...
        assertEq(usdcAvalanche.allowance(almProxy, address(centrifugeV3Vault)), 0);

        uint256 initialEscrowBal = usdcAvalanche.balanceOf(globalEscrow);

        assertEq(usdcAvalanche.balanceOf(almProxy),     1_000_000e6);
        assertEq(usdcAvalanche.balanceOf(globalEscrow), initialEscrowBal);

        assertEq(centrifugeV3Vault.pendingDepositRequest(REQUEST_ID, almProxy), 0);
        ...
        assertEq(usdcAvalanche.balanceOf(almProxy),     0);
        assertEq(usdcAvalanche.balanceOf(globalEscrow), initialEscrowBal + 1_000_000e6);

        assertEq(centrifugeV3Vault.pendingDepositRequest(REQUEST_ID, almProxy), 1_000_000e6);
    }
...
contract ForeignController_Centrifuge_ClaimDepositERC7540_Tests is Centrifuge_TestBase {
    ...
    function test_claimDepositERC7540_singleRequest() external {
        ...
        assertEq(centrifugeV3Vault.pendingDepositRequest(REQUEST_ID,   almProxy), 0);
        assertEq(centrifugeV3Vault.claimableDepositRequest(REQUEST_ID, almProxy), 0);
        ...
        assertEq(centrifugeV3Vault.pendingDepositRequest(REQUEST_ID,   almProxy), 0);
        assertEq(centrifugeV3Vault.claimableDepositRequest(REQUEST_ID, almProxy), 1_000_000e6);
    ...
    function test_claimDepositERC7540_multipleRequests() external {
        deal(address(usdcAvalanche), almProxy, 1_500_000e6);
        ...
        assertEq(centrifugeV3Vault.pendingDepositRequest(REQUEST_ID,   almProxy), 1_500_000e6);
        assertEq(centrifugeV3Vault.claimableDepositRequest(REQUEST_ID, almProxy), 0);
        ...
        assertEq(centrifugeV3Vault.pendingDepositRequest(REQUEST_ID,   almProxy), 0);
        assertEq(centrifugeV3Vault.claimableDepositRequest(REQUEST_ID, almProxy), 1_500_000e6);
...
contract ForeignController_Centrifuge_CancelDepositERC7540_Tests is Centrifuge_TestBase {
    ...
    function test_cancelCentrifugeDepositRequest() external {
        ...
        assertEq(centrifugeV3Vault.pendingDepositRequest(REQUEST_ID,       almProxy), 1_000_000e6);
        assertEq(centrifugeV3Vault.pendingCancelDepositRequest(REQUEST_ID, almProxy), false);

        vm.prank(ALM_RELAYER);
        foreignController.cancelCentrifugeDepositRequest(address(centrifugeV3Vault));

        assertEq(centrifugeV3Vault.pendingDepositRequest(REQUEST_ID,       almProxy), 1_000_000e6);
        assertEq(centrifugeV3Vault.pendingCancelDepositRequest(REQUEST_ID, almProxy), true);
    }
...
contract ForeignController_Centrifuge_ClaimCancelDeposit_Tests is Centrifuge_TestBase {
    ...
    function test_claimCentrifugeCancelDepositRequest() external {
        ...
        assertEq(centrifugeV3Vault.pendingDepositRequest(REQUEST_ID,         almProxy), 0);
        assertEq(centrifugeV3Vault.pendingCancelDepositRequest(REQUEST_ID,   almProxy), false);
        assertEq(centrifugeV3Vault.claimableCancelDepositRequest(REQUEST_ID, almProxy), 0);
        ...
        assertEq(centrifugeV3Vault.pendingDepositRequest(REQUEST_ID,         almProxy), 0);
        assertEq(centrifugeV3Vault.pendingCancelDepositRequest(REQUEST_ID,   almProxy), false);
        assertEq(centrifugeV3Vault.claimableCancelDepositRequest(REQUEST_ID, almProxy), 1_000_000e6);
...
contract ForeignController_Centrifuge_RequestRedeemERC7540_Tests is Centrifuge_TestBase {
    ...
    function test_requestRedeemERC7540() external {
        ...
        assertEq(centrifugeV3Vault.pendingRedeemRequest(REQUEST_ID, almProxy), 0);
        ...
        assertEq(centrifugeV3Vault.pendingRedeemRequest(REQUEST_ID, almProxy), shares);
    }
...
contract ForeignController_Centrifuge_ClaimRedeemERC7540_Tests is Centrifuge_TestBase {
    ...
    function test_claimRedeemERC7540_singleRequest() external {
        ...
        assertEq(centrifugeV3Vault.pendingRedeemRequest(REQUEST_ID,   almProxy), 0);
        assertEq(centrifugeV3Vault.claimableRedeemRequest(REQUEST_ID, almProxy), 0);
        ...
        assertEq(centrifugeV3Vault.pendingRedeemRequest(REQUEST_ID,   almProxy), 1_000_000e6);
        assertEq(centrifugeV3Vault.claimableRedeemRequest(REQUEST_ID, almProxy), 0);
        ...
        assertEq(centrifugeV3Vault.pendingRedeemRequest(REQUEST_ID,   almProxy), 0);
        assertEq(centrifugeV3Vault.claimableRedeemRequest(REQUEST_ID, almProxy), 1_000_000e6);
        ...
        assertEq(usdcAvalanche.balanceOf(almProxy),   2_000_000e6);
    ...
    function test_claimRedeemERC7540_multipleRequests() external {
        ...
        assertEq(centrifugeV3Vault.pendingRedeemRequest(REQUEST_ID,   almProxy), 1_500_000e6);
        assertEq(centrifugeV3Vault.claimableRedeemRequest(REQUEST_ID, almProxy), 0);
        ...
        assertEq(centrifugeV3Vault.pendingRedeemRequest(REQUEST_ID,   almProxy), 0);
        assertEq(centrifugeV3Vault.claimableRedeemRequest(REQUEST_ID, almProxy), 1_500_000e6);
        ...
        assertEq(usdcAvalanche.balanceOf(poolEscrow), 0);
        assertEq(usdcAvalanche.balanceOf(almProxy),   3_000_000e6);
...
contract ForeignController_Centrifuge_CancelRedeemRequest_Tests is Centrifuge_TestBase {
    ...
    function test_cancelCentrifugeRedeemRequest() external {
        ...
        assertEq(centrifugeV3Vault.pendingRedeemRequest(REQUEST_ID,       almProxy), shares);
        assertEq(centrifugeV3Vault.pendingCancelRedeemRequest(REQUEST_ID, almProxy), false);

        vm.prank(ALM_RELAYER);
        foreignController.cancelCentrifugeRedeemRequest(address(centrifugeV3Vault));

        assertEq(centrifugeV3Vault.pendingRedeemRequest(REQUEST_ID,       almProxy), shares);
        assertEq(centrifugeV3Vault.pendingCancelRedeemRequest(REQUEST_ID, almProxy), true);
    }
...
contract ForeignController_Centrifuge_ClaimCancelRedeemRequest_Tests is Centrifuge_TestBase {
    ...
    function test_claimCentrifugeCancelRedeemRequest() external {
        ...
        assertEq(centrifugeV3Vault.pendingRedeemRequest(REQUEST_ID,         almProxy), 0);
        assertEq(centrifugeV3Vault.pendingCancelRedeemRequest(REQUEST_ID,   almProxy), false);
        assertEq(centrifugeV3Vault.claimableCancelRedeemRequest(REQUEST_ID, almProxy), 0);
        ...
        assertEq(centrifugeV3Vault.pendingRedeemRequest(REQUEST_ID,         almProxy), 0);
        assertEq(centrifugeV3Vault.pendingCancelRedeemRequest(REQUEST_ID,   almProxy), false);
        assertEq(centrifugeV3Vault.claimableCancelRedeemRequest(REQUEST_ID, almProxy), shares);
