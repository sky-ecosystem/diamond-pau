...
interface IControllerLike {
    ...
    function getCollectRateLimitKey(address facility, address gem) external pure returns (bytes32);

    function getSubscribeRateLimitKey(address facility, address gem) external pure returns (bytes32);

    function getWithdrawRateLimitKey(address facility, address gem) external pure returns (bytes32);
...
contract Controller_NFATPrimeFacet_Tests is Integration_TestBase {
    ...
    function test_getWithdrawRateLimitKey() external {
        ...
        assertEq(
            controller.getWithdrawRateLimitKey(facility, gem),
            makeAddressAddressKey(keyPrefix, facility, gem)
        );
    }
    ...
    function test_getCollectRateLimitKey() external {
        ...
        assertEq(
            controller.getCollectRateLimitKey(facility, gem),
            makeAddressAddressKey(keyPrefix, facility, gem)
        );
    }
