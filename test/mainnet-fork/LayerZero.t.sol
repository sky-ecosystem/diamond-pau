...
contract MainnetController_LayerZero_TransferToken_Tests is LayerZero_TestBase {
    ...
    function test_quoteTransferLayerZero_revertsIfOFTQuoteWritesStorage() external {
        address oft = address(new MockOFT());

        bytes32 recipient = bytes32(type(uint256).max);

        vm.prank(SPARK_PROXY);
        mainnetController.setLayerZeroRecipient(DESTINATION_ENDPOINT_ID, recipient);

        vm.expectRevert();
        mainnetController.quoteTransferLayerZero(oft, 4444, DESTINATION_ENDPOINT_ID);
    }
