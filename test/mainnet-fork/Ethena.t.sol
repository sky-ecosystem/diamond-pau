...
contract MainnetController_Ethena_SetDelegatedSigner_Tests is Ethena_TestBase {
    ...
    function test_setDelegatedSigner() external {
        ...
        assertEq(IEthenaMinterLike(ETHENA_MINTER).delegatedSigner(signer, almProxy), 0);  // REJECTED
        ...
        assertEq(IEthenaMinterLike(ETHENA_MINTER).delegatedSigner(signer, almProxy), 1);  // PENDING
    }
...
contract MainnetController_Ethena_PrepareUSDEMint_Tests is Ethena_TestBase {
    ...
    function test_prepareUSDEMint() external {
        assertEq(usdc.allowance(almProxy, ETHENA_MINTER), 0);
        ...
        assertEq(usdc.allowance(almProxy, ETHENA_MINTER), 100e6);
    }
...
contract MainnetController_Ethena_PrepareUSDEBurn_Tests is Ethena_TestBase {
    ...
    function test_prepareUSDEBurn() external {
        assertEq(usde.allowance(almProxy, ETHENA_MINTER), 0);
        ...
        assertEq(usde.allowance(almProxy, ETHENA_MINTER), 100e18);
    }
...
contract MainnetController_Ethena_CooldownAssetsSUSDE_Tests is Ethena_TestBase {
    ...
    function test_cooldownAssetsSUSDE_rateLimitBoundary() external {
        ...
        deal(address(susde), almProxy, 100e18);
    ...
    function test_cooldownAssetsSUSDE() external {
        ...
        assertEq(susde.balanceOf(almProxy), 100e18);
...
contract MainnetController_Ethena_CooldownSharesSUSDE_Tests is Ethena_TestBase {
    ...
    function test_cooldownSharesSUSDE_zeroMaxAmount() external {
        deal(address(susde), almProxy, 100e18);  // To get past call
    ...
    function test_cooldownSharesSUSDE_rateLimitBoundary() external {
        deal(address(susde), almProxy, 100e18);  // For success case
...
contract MainnetController_Ethena_E2ETests is Ethena_TestBase {
    ...
    function _simulateUSDEMint(uint256 amount) internal {
        ...
        usdc.transferFrom(almProxy, ETHENA_MINTER, amount);

        deal(address(usde), almProxy, amount * 1e12);
    }
    ...
    function _simulateUSDEBurn(uint256 amount) internal {
        vm.prank(ETHENA_MINTER);
        usde.transferFrom(almProxy, ETHENA_MINTER, amount);

        deal(address(usdc), almProxy, amount / 1e12);
    }
    ...
    function test_ethena_e2eFlowUsingAssets() external {
        ...
        assertEq(usdc.allowance(almProxy, ETHENA_MINTER), 1_000_000e6);
        ...
        assertEq(usde.allowance(almProxy, address(susde)), 0);

        assertEq(susde.convertToAssets(susde.balanceOf(almProxy)), 0);
        ...
        assertEq(susde.convertToAssets(susde.balanceOf(almProxy)), 500_000e18 - 1);  // Rounding
        ...
        assertEq(usde.allowance(almProxy, ETHENA_MINTER), 1_000_000e18 - 1);
    ...
    function test_ethena_e2eFlowUsingShares() external {
        ...
        assertEq(susde.balanceOf(almProxy), susdeShares);
