...
contract Controller_Tests is Test {
    ...
    function test_addDispatch_invalidDispatch() external {
        _expectAndMockAccessControlCall(admin, true);

        vm.expectRevert(IController.InvalidDispatch.selector);
        vm.prank(admin);
        controller.addDispatch(bytes4(0), IController.Dispatch(address(0), bytes4(0)));
    }
    ...
    function test_addDispatch_dispatchAlreadyEnabled() external {
        ...
        vm.prank(admin);
        controller.addDispatch(0x12345678, IController.Dispatch(makeAddr("facet"), bytes4(0)));
    }
    ...
    function test_addDispatches_invalidDispatch() external {
        ...
        dispatches[0] = IController.Dispatch(makeAddr("facet"), bytes4(0));
    ...
    function test_addDispatches_dispatchAlreadyEnabled() external {
        ...
        IController.Dispatch[] memory dispatches = new IController.Dispatch[](2);
        dispatches[0] = IController.Dispatch(makeAddr("facet1"), bytes4(0));
        dispatches[1] = IController.Dispatch(makeAddr("facet2"), bytes4(0));
    ...
    function test_addWire_invalidDispatch() external {
        _expectAndMockAccessControlCall(admin, true);
    ...
    function test_addWire_dispatchAlreadyEnabled() external {
        ...
        vm.expectRevert(abi.encodeWithSelector(IController.DispatchAlreadyEnabled.selector, bytes4(0x12345678)));
        vm.prank(admin);
        controller.addWire(makeAddr("facet"), IController.Wire(0x12345678, bytes4(0)));
    }
    ...
    function test_addWires_invalidDispatch() external {
